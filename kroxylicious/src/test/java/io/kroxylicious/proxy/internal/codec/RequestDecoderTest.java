        ...
        assertEquals(12,
                exactlyOneFrame_decoded(apiVersion,
                        ...
                        new KafkaRequestDecoder(
                                DecodePredicate.forFilters(KrpcFilter.of((ApiVersionsRequestFilter) (request, context) -> context.forwardRequest(request.body())))),
                        DecodedRequestFrame.class,
                        (RequestHeaderData header) -> header),
                "Unexpected correlation id");
    }

    private static class TargetingOverrideFilter implements ApiVersionsRequestFilter, RequestTargeter {

        @Override
        public void onApiVersionsRequest(DecodedRequestFrame<ApiVersionsRequestData> requestFrame, KrpcFilterContext context) {
            context.forwardRequest(requestFrame.body());
        }
    ...
    public void testApiVersionsExactlyOneFrame_opaque(short apiVersion) throws Exception {
        assertEquals(12,
                exactlyOneFrame_encoded(apiVersion,
                        ...
                        AbstractCodecTest::exampleRequestHeader,
                        AbstractCodecTest::exampleApiVersionsRequest,
                        new KafkaRequestDecoder(DecodePredicate.forFilters(KrpcFilter.of(
                                new TargetingOverrideFilter()))),
