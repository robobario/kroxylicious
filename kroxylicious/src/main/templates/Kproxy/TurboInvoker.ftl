...
<#list messageSpecs as messageSpec>
    ...
    public TurboInvoker(KrpcFilter filter){
        this.inner = filter;
...
throw new IllegalStateException("Unsupported RPC " + decodedFrame.apiKey());
    ...
    public boolean shouldDeserializeRequestOuter(ApiKeys apiKey, short apiVersion) {
        return requestKeys.contains(apiKey) && inner.shouldDeserializeRequest(apiKey, apiVersion);
    }

    public boolean shouldDeserializeResponseOuter(ApiKeys apiKey, short apiVersion) {
        return responseKeys.contains(apiKey) && inner.shouldDeserializeResponse(apiKey, apiVersion);
    }
