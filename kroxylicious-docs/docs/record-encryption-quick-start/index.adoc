...
:context: record-encryption-quick-start
:quick-start: record-encryption

[id="quick-start-{context}"]
= Record Encryption Quick Start
...
This quick start guide will show you how to do that on Kubernetes ... from scratch ... without external dependencies.
...
WARNING: You *really don't* want to use this quick start to deploy an environment that will be used in *production*.
The quick start deliberately keeps things quick and simple... there's no authentication, no TLS, no redundancy... Development purposes only - please!
Refer to the documentation for Kroxylicious, Strimzi and your KMS for production best practices.

== Overview

This diagram shows the important components that will be deployed by the quick start.
...
image::{realimagesdir}/quick-start-record-encryption.svg["Diagram showing the important kubernetes resources deployed by the quick start and the flows between them"]
...
Let's get the Kubernetes Cluster up and running.  Minikube defaults work just fine for this quick start.
...
NOTE: After installation, the LocalStack Helm chart prompts you to "Get the application URL". For this quick start, you can ignore these steps.
====
...
The filter is configured to expect a key to exist in the KMS with the name `KEK-<topic name>`.
We are going to be using a topic called `trades` so we will create a key that can be referred to using the name `KEK-trades`.
