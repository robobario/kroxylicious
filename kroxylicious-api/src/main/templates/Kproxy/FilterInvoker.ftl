...
<#assign
  ...
  filterResultClass="CompletionStage<${messageSpec.type?lower_case?cap_first}FilterResult<ApiMessage>>"
...
class ${filterInvokerClass} implements FilterInvoker {
    ...
    @Override
    public ${filterResultClass} on<#if messageSpec.type?lower_case == 'response'>Response<#else>Request</#if>(ApiKeys apiKey, short apiVersion, ${headerClass} header, ApiMessage body, KrpcFilterContext<ApiMessage> filterContext) {
        return filter.on${messageSpec.name}(apiVersion, header, (${messageSpec.name}Data) body, KrpcFilterContext.toSpecificContext(filterContext)).thenApply(<#if messageSpec.type?lower_case == 'response'>Response<#else>Request</#if>FilterResult::asApiMessageResult);
    }
}